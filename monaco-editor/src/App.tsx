...
const App = () => {
  ...
  const fetchFile = async () => {
    const response = await fetch(
      `http://localhost:8082/api/get-file/${fileName}`
    );
  ...
  return (
    <div style={{ width: "100vw", marginTop: "0", height: "100vh" }}>
      ...
      <MonacoEditor filePath={fileName} value={code} onChange={handleChange} />
      <div style={{marginTop:'40px'}}>
      <button onClick={sendPatchesToBackend}>Apply Patches to Backend</button>
            </div>
